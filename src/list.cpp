// TODO Change File Class to hold fs::path, add better symlink support, show
// symlinks with a different color, greenish for dirs orangish for files. and
// add an '@' maybe?
...
int main(int argc, char **argv)
{
	...
	for(const auto &entry: std::filesystem::directory_iterator(directory))
	{
		...
		if(entry.is_symlink() &&
		   std::filesystem::is_directory(std::filesystem::read_symlink(entry)))
			file = new File(
				path, path.rfind('/') + 1U, entry.is_directory(), 4096UL);
		else
			file = new File(path,
							path.rfind('/') + 1U,
							entry.is_directory(),
	...
	if(long_list)	 // -l option
	{
		for(const File &item: dir)
		{
			...
			std::string m_time(ctime(&modify));	   // Convert time_t into a
												   // prettier string equivelant
